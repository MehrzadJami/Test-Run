...
// Canonical ExtractionResult for the chemostat demo. Mirrors the normalized
// rows below 1:1 and is also written to the new `raw_extraction_json` column
// so the model card detail page can surface model_card metadata
// (inputs/outputs/control_variables/missing_information) and per-item
// confidences for the showcase demo.
...
/**
 * Idempotent seed: inserts the chemostat demo project on first boot only.
 * If the project already exists (matched by name), backfill the new
 * `raw_extraction_json` column on its extraction row when missing so the
 * model card detail page can show model_card metadata for the showcase demo.
 */
export async function seedDemoData(): Promise<void> {
  ...
  if (existing.length > 0) {
    ...
    // One-time backfill: if the chemostat extraction predates the
    // raw_extraction_json column, attach the canonical payload now.
    ...
    if (updated.length > 0) {
      logger.info(
        { projectId, count: updated.length },
        "Seed: backfilled raw_extraction_json on existing chemostat demo",
      );
    } else {
      logger.info(
        { projectId },
        "Seed: chemostat demo project already present, skipping",
      );
    }
    return;
  }

  logger.info("Seed: inserting chemostat demo project");
  ...
  await db.transaction(async (tx) => {
    ...
    if (!source) throw new Error("Failed to insert seed source document");
    ...
    if (!extraction) throw new Error("Failed to insert seed extraction");
    ...
    await tx.insert(variablesTable).values([
      ...
      },
      {
        ...
        unit: "mg/L",
        role: "state",
        sourceQuote: "S is the residual concentration of the limiting nutrient.",
      },
      {
        extractionId: extraction.id,
        ordinal: 2,
    ...
    logger.info(
      { projectId: project.id, extractionId: extraction.id },
      "Seed: chemostat demo project inserted",
    );
  });
}
